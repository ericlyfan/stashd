...
- When matched via body text, the response is a `SearchHit` carrying a `snippet` — ±80 chars around the first match — which the UI renders in italics on result cards/rows so you can see _why_ something matched.
...
- Abandoned uploads leave their temp dir (and possibly an `.extracted.txt` sidecar) under `data/temp/` indefinitely — there's no cleanup job; several stale job dirs exist today.
